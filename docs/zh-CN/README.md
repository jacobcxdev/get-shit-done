...
> This translation may lag behind the English docs. Install and update this fork from source only; follow the root [README.md](../../README.md) and [docs/manual-update.md](../manual-update.md). The npm package `get-shit-done-cc` is a compatibility/upstream name unless future fork publishing is documented.
...
```bash
# Claude Code
node bin/install.js --claude --global   # 安装到 ~/.claude/
node bin/install.js --claude --local    # 安装到 ./.claude/

# OpenCode
node bin/install.js --opencode --global # 安装到 ~/.config/opencode/

# Gemini CLI
node bin/install.js --gemini --global   # 安装到 ~/.gemini/

# Kilo
node bin/install.js --kilo --global     # 安装到 ~/.config/kilo/
node bin/install.js --kilo --local      # 安装到 ./.kilo/

# Codex
node bin/install.js --codex --global    # 安装到 ~/.codex/
node bin/install.js --codex --local     # 安装到 ./.codex/

# 所有运行时
node bin/install.js --all --global      # 安装到所有目录
```
...
**命令没有按预期工作？**
- 运行 `/gsd-help` 验证安装
- 从此 fork 的源码 checkout 中重新运行 `node bin/install.js` 重新安装
...
OpenCode、Gemini CLI、Kilo 和 Codex 由此 fork 的源码安装器原生支持。`get-shit-done-cc` 是兼容/upstream npm 名称。
...
<a href="https://star-history.com/#jacobcxdev/get-shit-done&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=jacobcxdev/get-shit-done&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=jacobcxdev/get-shit-done&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=jacobcxdev/get-shit-done&type=Date" />
 </picture>
</a>
